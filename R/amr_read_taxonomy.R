...
#' @description given raw data for AMR and WIMP, provides full AMR and toxon info for those reads that assign to both
...
amr_read_taxonomy <- function(path.to.wimp.files, path.to.amr.files, coveragenumber, keepSNP){
  read_in_amr_files <- function(path.to.amr.files, coveragenumber=80){
    ...
    amr.rawdata.reduced <- amr.rawdata[, list(csvname, barcode, coverage, URL, read_id)]
    ...
    amr.rawdata.reduced.subset <- amr.rawdata.reduced[, list(sampleID, coverage, CVTERMID, read_id)]
    amr.rawdata.reduced.subset <- amr.rawdata.reduced.subset[coverage %between% c(coveragenumber, 100)]
    amr.rawdata.reduced.subset <- amr.rawdata.reduced.subset[, list(read_id, sampleID, CVTERMID)]
  }
  ...
  combo_wimp_amr <- merge(x = try_read_id, y = wimpfiles, by = "read_id", all.x = TRUE)
  ...
  percentage.classifed <- round((classifed_amr_gene_number/total_amr_gene_number*100), digits = 2)
  ...
  merged.data <- merge(x = combo_classifed_only, y = CARD_taxonomy, by = "CVTERMID", all.x = TRUE)
  taxa_short <- merged.data[, c("read_id", "CVTERMID","Drug Class", "AMR Gene Family", "Resistance Mechanism", "ARO Name", "mutationassociated", "taxID")]
  ...
  merged.data.both <- merge(x = taxa_short, y = full.taxon.wimp.dt, by = "taxID", all = TRUE, allow.cartesian=TRUE)
  ...
  merged.data.both <- merged.data.both[, c("read_id", "CVTERMID","Drug Class",
                                           ...
                                           "ARO Name", "mutationassociated", "taxID", "superkingdom",
