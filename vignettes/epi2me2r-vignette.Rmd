...
epi2me2r is meant to full a fully automated methods to take raw csv of CARD ARMA and
...
Prior to starting, making sure the metadata file is formatted appropriately and in the correct format will ensure your data is imported correctly. You can use one metadata file for both your AMR and WIMP samples or separate. Both options are described below. 
...
epi2me.metadata <- read.csv("../inst/extdata/example_metadata.csv")
head(epi2me.metadata)
```

#### Individual Metadata file

If you are just importing WIMP or CARD ARMA files, you do not need all the meta data assoicated with the other workflow. 
...
Even if you are just processing one type of data, both arma and wimp information can be included in the metadata (as seen in the combo metadata)


## Full Automated data import

For both AMR and WIMP data, the raw csv's downloaded from the epi2me website need to be in their own directory (without any other files). Note that if you are processing both WIMP and ARM data you will need **two** directories one for each set of data. 
...
```{r amr phylo}
ps.amr.object <- amr_raw_to_phyloseq(path.to.amr.files = "../inst/extdata/example_amr_data/",
...
```{r amr mgs}
mgs.amr.object <- amr_raw_to_metagenomeseq(path.to.amr.files = "../inst/extdata/example_amr_data/",
                                     metadata = epi2me.metadata,
                                     coveragenumber = 80, 
                                     keepSNP = FALSE)
mgs.amr.object
...
ps.wimp.object <- wimp_raw_to_phyloseq(path.to.wimp.files = "../inst/extdata/example_wimp_data/",
                                     metadata = epi2me.metadata,
...
mgs.wimp.object <- wimp_raw_to_metagenomeseq(path.to.wimp.files = "../inst/extdata/example_wimp_data/",
                                     metadata = epi2me.metadata,
...
```{r amr read in}
amr.count.table <- read_in_amr_files(path.to.amr.files = "../inst/extdata/example_amr_data/",
                                     coveragenumber = 80, 
                                     keepSNP = FALSE)
...
```{r wimp read in}
wimp.count.table <- read_in_wimp_files(path.to.wimp.files = "../inst/extdata/example_wimp_data/")
...
Once function that can be useful but does not fit into the other sections is `amr_read_taxonomy` which matched any classified AMR read with the phylogenetic taxonomy (if it is assigned) using read_id. To use this you would need the following:

- `path.to.amr.files` *required*: the path to the raw csv files (i.e. "Desktop/raw_amr_data/")
- `path.to.wimp.files` *required*: the path to the raw csv files (i.e. "Desktop/raw__wimp_data/")
- `coveragenumber` *optional* : the total length of the gene that must be present for it to be included in the count table, the default is 80%; this argument takes any number from 1 to 99. Default is `80`.


```{r amr read, eval=F}
amr.read.classification <- amr_read_taxonomy(path.to.amr.files = "../inst/extdata/example_amr_data/",
                                             path.to.wimp.files = "../inst/extdata/example_wimp_data/")
```
